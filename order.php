...
<body>
                ...
                <div class="row">
                    ...
                    foreach ($product as $prod) {
                        ...
                        <div class="card col-md-3 m-2" style="width: 18rem;">
                            <img height="200px" width="100px" src="assets/photo/<?= $prod['photo'] ?>"
                            ...
                            <ul class="list-group list-group-flush">
                                ...
                                <?php if ($prod['discount'] != null && $prod['discount'] > 0) {
                                    ...
                                    $prc = (int) $prod['price'];
                                    $discount = intval(($disc / 100) * $prc);
                                    $discountedPrice = intval($prc - $discount);
                                    ?>
                                    <li class="list-group-item">PKR. <s>
                                            <?= number_format($prod['price'], 2, ",", ".") ?>
                                        </s></li>
                                    <li class="list-group-item">PKR.
                        ...
                        <tbody>
                            ...
                            foreach ($product as $prod) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?= $prod['product_name'] ?>
                                        <input type="hidden" name="price" id="price<?= $prod['id'] ?>"
                                            value="<?= $prod['price'] ?>">
                                        <input type="hidden" name="disc" id="disc<?= $prod['id'] ?>"
                                            value="<?= $prod['discount'] ?>">
                                    </td>

                                    <?php if ($prod['subtotal'] == NULL) { ?>
                                        <td><input type="number" name="unit" id="unit<?= $prod['id'] ?>"
                                                onblur="getDisc(<?= $prod['id'] ?>)"></td>
                                    ...
                                    <?php } else { ?>
                                        <td><input type="number" name="unit" id="unit<?= $prod['id'] ?>"
                                                value="<?= $prod['unit_a'] ?>" readonly></td>
                                        <td><input type="number" name="subtotal" id="subtotal<?= $prod['id'] ?>"
                                                value="<?= $prod['subtotal'] ?>" readonly></td>
                                        <td>
                                            <a onclick="deletex(<?= $prod['id'] ?>)" class="btn btn-sm btn-secondary">Delete</a>
                                        </td>
                                    <?php } ?>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (disc != null && disc > 0) {
                        ...
                        discount = parseInt((disc / 100) * price)
                        discountedPrice = parseInt(price - discount);
                        txt = `<s>${price}</s>`
                        txt1 = `${discountedPrice}`
                    } else {
                        txt = `${price}`;
                        txt1 = '';
                    }
        ...
        function add(id) {
            ...
            let unit = $(`#unit${id}`).val()
            let conf = confirm('Do you want to add this item to your cart?');
            if (conf) {
                $.ajax({
                    type: "POST",
                    url: "add_temp.php",
